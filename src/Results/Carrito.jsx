...
export default function Carrito() {
  ...
  const totalAmount = useMemo(() => {
    return cart.reduce((total, item) => total + item.precioFinal, 0);
  }, [cart]);
      ...
      <Box position="relative" minHeight="100vh" bg="#F5F5F5">
        ...
        >
          <Flex justifyContent="flex-start" alignItems="center" gap=".4rem">
            ...
            <Text fontSize="24px" fontWeight="bold">{totalAmount.toFixed(2)}</Text>
          </Flex>
            <Button as={NavLink} to="" w="100%" borderRadius={"24px"} bgColor="#CA0017" color="white" fontSize="1rem" height="3rem"> Continuar</Button>
        </Box>
      </Box>
    </ChakraProvider>
  );
